...
The MVP does not run its own editorial scheduler or continuously generate in the background. An explicit CLI command may generate today's edition, and asking to view today generates it when none exists. Users may invoke the same generation command with an external scheduler such as `cron` or `launchd`; an optional local delivery-surface server may remain available without changing the edition lifecycle.

An optional Daily Edition Delivery may present the canonical edition through a user-chosen surface. The email adapter sends the standalone Edition Review from a user-authorized Gmail account back to that same account. The private-web adapter publishes only the current Edition Review at a stable tailnet-restricted URL; it exposes no dated archive, and email may link to it when configured. Delivery is presentation, not a newsletter subscription or unread queue: one successfully recorded email delivery is not sent again, delivery failures create no user-facing debt or pressure, and a failed delivery never changes the immutable edition. A network failure after Gmail accepts a message but before local receipt recording may cause a duplicate retry; Musement prefers this narrow possibility over requesting Gmail read access solely for remote deduplication.
